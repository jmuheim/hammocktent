...
task :notify_rollbar do
  ...
  queue "curl https://api.rollbar.com/api/1/deploy/ -F access_token=#{rollbar_token} -F environment=#{rails_env} -F revision=#{revision} -F local_username=#{local_user} --silent >/dev/null 2>&1" # TODO: Prevent display of CURL output! https://github.com/mina-deploy/mina/issues/268
end
